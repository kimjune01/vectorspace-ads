...
If you're running an ad auction in this space — using [power diagrams](https://kimjune01.github.io/power-diagrams-for-ads/) to allocate territory — then the advertiser's job is to specify a *region* of this space. Not a keyword. Not a list. A region. And that raises a UX problem that has no analog in the keyword world.
...
So you need an interface that lets advertisers navigate embedding space without ever touching coordinates. The answer, we think, is semantic hill-climbing.
...
You can see your neighbors' territories. You can see where the high-density regions are. You can see that increasing your bid will expand your boundary at the expense of the advertiser next to you, and you can estimate the cost before you do it.

This is more transparent than any keyword auction. In keyword systems, you bid on a word and discover your effective CPM after the fact. Here, the competitive dynamics are visible on the map. The price of a region is a function of who else is nearby and how much traffic passes through — and you can see both before you commit.
...
![Restriction zones in the power diagram](../shared/diagrams/08_restriction_zones.png)
...
The most powerful thing about spatial ad buying is that competition becomes visible.

In keyword auctions, competition is invisible. You bid $2 on "running shoes," someone else bids $2.50, and you lose. You can raise your bid, but you can't see who you're competing against or what they're doing. The auction is a series of independent, opaque events.

In embedding space, competition is spatial. If you can see the territory map, you can see that Nike owns the running-shoe region, that GNC is encroaching from the nutrition side, and that there's an underserved gap in the home-fitness area where no one has planted a flag. You can move your targeting locus into that gap. You can specialize your reach parameter to be narrow along the topic axis and broad along the intent axis, creating an elongated territory that captures high-intent users across multiple subtopics.

The price signals reinforce this. The CPM estimates shown on each direction card reflect both impression density (more traffic = higher prices) and competitive pressure (more nearby advertisers = higher second-price floor). An uncrowded region with decent traffic is a deal. A crowded region near a high bidder is expensive. The advertiser sees this *before* committing, not in a retrospective analytics dashboard.

## An Open Design Problem

This is a proof of concept, not a production system. Real embedding spaces have hundreds of dimensions, not two. The 2D projection we use for visualization is a useful lie — it preserves some structure but inevitably distorts distances and neighborhoods. The semantic anchors are hand-authored, not derived from actual embedding clusters. The CPM estimates are approximations, not auction outcomes.
...
These are solvable problems. They're UX problems, not mathematical ones. The auction mechanism — power diagrams with additively weighted Voronoi cells — is well-defined and has good incentive properties regardless of dimensionality. The question is how to build an interface that lets humans make good decisions in a space they can't see.

We think semantic hill-climbing is the right starting point. Try the [interactive prototype](/vectorspace-ads/) and decide for yourself.
